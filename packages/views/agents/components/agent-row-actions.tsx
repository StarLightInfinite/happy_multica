...
export function AgentRowActions({
  ...
  const handleArchive = async () => {
    try {
      ...
      toast.error(e instanceof Error ? e.message : t("agents", "failedArchiveAgent"));
    }
  };
  ...
  const handleRestore = async () => {
    try {
      ...
      toast.error(e instanceof Error ? e.message : t("agents", "failedRestoreAgent"));
    }
  };
  ...
  const handleCancelTasks = async () => {
    try {
      ...
      toast.success(
        cancelled === 0
          ? t("agents", "noActiveTasks")
          : `${t("agents", "cancelledTasks").replace("{count}", String(cancelled))}`,
      );
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t("agents", "failedCancelTasks"));
    }
  };
      ...
      {confirmCancel && (
        ...
        >
          ...
          >
            <AlertDialogHeader>
              <AlertDialogTitle>
                {t("agents", "cancelTasksTitle").replace("{name}", agent.name)}
              </AlertDialogTitle>
              <AlertDialogDescription>
                {describeCancelImpact(runningCount, queuedCount, t)}
                {runningCount > 0 && (
                  <>{" "}{t("agents", "cancelTasksDesc")}</>
                )}{" "}
                {t("agents", "cancelTasksDesc2")}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
            ...
            <AlertDialogHeader>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <AlertDialogTitle>
                    {t("agents", "archiveTitle").replace("{name}", agent.name)}
                  </AlertDialogTitle>
                  <AlertDialogDescription>
                    {t("agents", "archiveDesc")}
                  </AlertDialogDescription>
                </div>
              </div>
            </AlertDialogHeader>
...
function describeCancelImpact(running: number, queued: number, t: TranslateFn): string {
  ...
  if (running === 0 && queued === 0) {
    return t("agents", "noActiveTasks");
  }
  const parts: string[] = [];
  if (running > 0) parts.push(`${running} running`);
  if (queued > 0) parts.push(`${queued} queued`);
  return t("agents", "willCancelTasks")
